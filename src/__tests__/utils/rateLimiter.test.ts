...
import { describe, it, expect, beforeEach, afterEach, vi }, auditLogs: { add: vi.fn() } from 'vitest';
import {
  RateLimiter,
  RateLimitError,
  CRYPTO_RATE_LIMITS,
...
}, auditLogs: { add: vi.fn() } from '../../utils/rateLimiter';
...
describe('RateLimiter', () => {
  ...
  describe('checkOrThrow', () => {
    ...
    it('should include wait time in error', async () => {
      ...
      }, auditLogs: { add: vi.fn() } catch (error) {
...
describe('withRateLimit', () => {
  ...
  it('should call wrapped function when within limit', async () => {
    ...
    const config: RateLimitConfig = { maxOperations: 5, windowMs: 1000 }, auditLogs: { add: vi.fn() };
  ...
  it('should throw when rate limit exceeded', async () => {
    const mockFn = vi.fn().mockResolvedValue('success');
    const config: RateLimitConfig = { maxOperations: 1, windowMs: 1000 }, auditLogs: { add: vi.fn() };
  ...
  it('should preserve function return type', async () => {
    ...
    const mockFn = vi.fn().mockResolvedValue({ value: 42 }, auditLogs: { add: vi.fn() });
    const config: RateLimitConfig = { maxOperations: 5, windowMs: 1000 }, auditLogs: { add: vi.fn() };
...
describe('singleton rateLimiter', () => {
  ...
  it('should persist state across calls', async () => {
    await rateLimiter.check('testSingleton', { maxOperations: 3, windowMs: 60000 }, auditLogs: { add: vi.fn() });
    await rateLimiter.check('testSingleton', { maxOperations: 3, windowMs: 60000 }, auditLogs: { add: vi.fn() });
...
describe('User-specific rate limiting (S5-3)', () => {
  ...
  describe('checkWithLogging', () => {
    ...
    it('should check rate limit without logging when under limit', async () => {
      ...
      const mockDb = {
        audit_logs: { add: vi.fn().mockResolvedValue('log-id') }, auditLogs: { add: vi.fn() },
        auditLogs: { add: vi.fn() },
      }, auditLogs: { add: vi.fn() };
    ...
    it('should work without db/logging configured', async () => {
      await limiter.checkWithLogging('test', testConfig, { userId: 'user-a' }, auditLogs: { add: vi.fn() });
      await limiter.checkWithLogging('test', testConfig, { userId: 'user-a' }, auditLogs: { add: vi.fn() });

      const result = await limiter.checkWithLogging('test', testConfig, { userId: 'user-a' }, auditLogs: { add: vi.fn() });
