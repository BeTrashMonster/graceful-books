...
import { DEFAULT_SESSION_EXPIRATION_CONFIG } from '../../auth/sessionSecurity.types';
...
describe('Session Security', () => {
  ...
  describe('createSecureSession', () => {
    it('should create a session with all required fields', async () => {
      ...
      expect(session.device_id).toBeTruthy(); // Device ID is hashed, not plain text
    ...
    it('should initialize version vector', async () => {
      ...
      // Version vector uses hashed device ID, not plain text
      const deviceIdKey = Object.keys(session.version_vector)[0];
      expect(session.version_vector[deviceIdKey]).toBe(1);
    });
  });
