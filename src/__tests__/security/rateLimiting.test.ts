 ...
 *
 * Tests for the sliding window rate limiter implementation to prevent abuse
 * and denial-of-service attacks (OWASP A04:2021 - Insecure Design).
 *
 * SECURITY FIX: Task S8-1 - Comprehensive rate limiting security tests
 *
 * Test Coverage:
 * - Basic rate limiting functionality
 * - Sliding window behavior
 * - User-specific rate limiting
 * - Security scenarios (brute force, data scraping, batch abuse)
 * - Error handling and logging
 *
 * Covers: S5-3 (Rate Limiting) from SECURITY_HARDENING_ROADMAP.md
 */
...
/**
 * Summary of Rate Limiting Test Coverage
 *
 * Functionality Tested:
 * ✓ Basic rate limiting (allow/block operations)
 * ✓ Sliding window behavior
 * ✓ Quota tracking and reporting
 * ✓ Enable/disable functionality
 * ✓ Clear operations (all/specific key)
 * ✓ checkOrThrow with error handling
 * ✓ User-specific rate limiting
 * ✓ Format wait time user messages
 *
 * Security Scenarios Tested:
 * ✓ Brute force login attack prevention (5 attempts/minute)
 * ✓ Data scraping prevention (100 requests/minute)
 * ✓ Batch query abuse prevention (10 requests/minute)
 * ✓ CPG calculation abuse prevention (50 requests/hour)
 * ✓ Legitimate rapid usage (within limits)
 *
 * Rate Limit Configurations:
 * ✓ CRYPTO_RATE_LIMITS (key derivation, batch encrypt, file encrypt, reencrypt)
 * ✓ SECURITY_RATE_LIMITS (login, data access, batch query, CPG calculation)
 *
 * Edge Cases:
