...
import { getKeyFingerprint } from '../../db/schema/backupPreferences.schema';
...
/**
 * Auto-key file name in the backup folder.
 * This key is the AUTHORITATIVE copy - the IndexedDB copy is a fallback.
 * IMPORTANT: This file is excluded from backup rotation/cleanup.
 */
export const AUTO_KEY_FILENAME = '.graceful-books-key';

/**
 * Result of writing the auto-key to the backup folder
 */
export interface WriteAutoKeyResult {
  success: boolean;
  error?: string;
  errorCode?: ErrorCode;
}
...
/**
 * Write the auto-key to the backup folder.
 * This is the AUTHORITATIVE copy of the key. The IndexedDB copy is a fallback.
 *
 * SECURITY: The key file travels with the backup files, ensuring that if the
 * device dies, the backups are still openable from the folder copy.
 *
 * @param autoKey - The base64-encoded 256-bit auto-key
 * @returns Result indicating success or failure with error details
 */
export async function writeAutoKeyToFolder(autoKey: string): Promise<WriteAutoKeyResult> {
  try {
    const fingerprint = await getKeyFingerprint(autoKey);
    console.log(`[Backup] Writing auto-key to folder, fingerprint: ${fingerprint}`);
    fileSystemLogger.info('Writing auto-key to backup folder', { fingerprint });

    // Check browser support
    const support = detectBrowserSupport();
    console.log('[Backup] Browser support check:', support);
    if (!support.supported) {
      console.error('[Backup] Browser not supported for file system access');
      return {
        success: false,
        error: 'Browser does not support file system access for auto-key storage',
        errorCode: ErrorCode.NOT_IMPLEMENTED,
      };
    }
    ...
    // Get stored directory handle
    const dirHandle = await retrieveDirectoryHandle();
    console.log('[Backup] Directory handle retrieved:', dirHandle ? dirHandle.name : 'null');
    if (!dirHandle) {
      console.error('[Backup] No backup folder configured');
      return {
        success: false,
        error: 'No backup folder configured. Please select a backup folder first.',
        errorCode: ErrorCode.VALIDATION_ERROR,
      };
    }
    ...
    // Verify permission
    const verification = await verifyDirectoryPermission(dirHandle);
    console.log('[Backup] Permission verification:', verification);
    if (!verification.granted) {
      console.error('[Backup] No permission to access backup folder');
      return {
        success: false,
        error: 'Cannot write auto-key: no permission to access backup folder',
        errorCode: ErrorCode.PERMISSION_DENIED,
      };
    }

    // Create the key file content
    // Format: JSON with metadata for clarity
    const keyFileContent = JSON.stringify(
      {
        ...
        note: 'This key encrypts your automatic backups. Keep it safe with your backup files.',
      },
      null,
      2
    );

    // Create or overwrite the key file
    let fileHandle: FileSystemFileHandle;
    try {
      fileHandle = await dirHandle.getFileHandle(AUTO_KEY_FILENAME, { create: true });
    } catch (error) {
      fileSystemLogger.error('Failed to create auto-key file handle', { error });
      return {
        success: false,
        error: 'Failed to create auto-key file in backup folder',
        errorCode: ErrorCode.DATABASE_ERROR,
      };
    }

    // Write the key
    console.log(`[Backup] Writing key file: ${AUTO_KEY_FILENAME}`);
    let writable: FileSystemWritableFileStream | null = null;
    try {
      writable = await fileHandle.createWritable();
      await writable.write(keyFileContent);
      await writable.close();
      writable = null;

      console.log(`[Backup] Key file written successfully: ${AUTO_KEY_FILENAME}`);
      fileSystemLogger.info('Auto-key written successfully to backup folder');
    } catch (error) {
      fileSystemLogger.error('Failed to write auto-key file', { error });
      ...
      return {
        success: false,
        error: 'Failed to write auto-key to backup folder. Your backups may not be recoverable without this key.',
        errorCode: ErrorCode.DATABASE_ERROR,
      };
    }
    ...
    fileSystemLogger.error('Unexpected error writing auto-key', { error });
    return {
      success: false,
      error: 'Unexpected error writing auto-key to backup folder',
      errorCode: ErrorCode.UNKNOWN_ERROR,
    };
  }
}
...
/**
 * Read the auto-key from the backup folder.
 * Used as the AUTHORITATIVE source; IndexedDB is fallback.
 *
 * @returns The auto-key if found, null otherwise
 */
export async function readAutoKeyFromFolder(): Promise<string | null> {
  try {
    fileSystemLogger.debug('Reading auto-key from backup folder');
    ...
    let fileHandle: FileSystemFileHandle;
    try {
      fileHandle = await dirHandle.getFileHandle(AUTO_KEY_FILENAME);
    } catch {
      // File doesn't exist
      fileSystemLogger.debug('Auto-key file not found in backup folder');
      return null;
    }
    ...
    try {
      ...
      if (parsed.type === 'graceful-books-auto-key' && parsed.key) {
        const fingerprint = await getKeyFingerprint(parsed.key);
        console.log(`[Backup] Read auto-key from folder, fingerprint: ${fingerprint}`);
        fileSystemLogger.info('Auto-key read successfully from backup folder', { fingerprint });
        return parsed.key;
      }

      fileSystemLogger.warn('Auto-key file has invalid format');
      return null;
    } catch (error) {
      fileSystemLogger.error('Failed to read auto-key file', { error });
      return null;
    }
  } catch (error) {
    fileSystemLogger.error('Unexpected error reading auto-key', { error });
    return null;
  }
}
