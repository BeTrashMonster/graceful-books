...
/**
 * Helper function to log entity creation
 * Convenience wrapper around createAuditLog for CREATE actions
 */
export async function logCreate(
  companyId: string,
  userId: string,
  ...
  await createAuditLog(
    {
      ...
      afterValues: JSON.stringify(afterValue),
...
/**
 * Helper function to log entity updates
 * Convenience wrapper around createAuditLog for UPDATE actions
 */
export async function logUpdate(
  companyId: string,
  userId: string,
  ...
  // Use provided changed fields or determine by comparing before and after
  const fields = changedFields || Object.keys(afterValue).filter(
    (key) => beforeValue[key] !== afterValue[key]
  )
  ...
  await createAuditLog(
    {
      ...
      entityId,
      action: 'UPDATE',
      beforeValues: JSON.stringify(beforeValue),
      afterValues: JSON.stringify(afterValue),
...
/**
 * Helper function to log entity deletion
 * Convenience wrapper around createAuditLog for DELETE actions
 */
export async function logDelete(
  companyId: string,
  userId: string,
  ...
  await createAuditLog(
    {
      ...
      changedFields: Object.keys(beforeValue),
      ipAddress: null,
      userAgent: null,
    },
    context
