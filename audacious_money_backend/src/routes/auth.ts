...
import { generateUniqueFunCompanyId } from '../utils/funCompanyId.js';
...
auth.post('/signup', validate(signupSchema), async (c) => {
  ...
  try {
    ...
    // Generate fun company ID (e.g., ocean_elephant_614)
    console.log('[Auth] Generating fun company ID...');
    const funCompanyId = await generateUniqueFunCompanyId(db);
    console.log('[Auth] Generated fun company ID:', funCompanyId);
    ...
    // Note: Using fun company ID as user ID for memorable, brand-aligned IDs
...
auth.post('/beta-signup', validate(signupSchema), async (c) => {
  ...
  try {
    ...
    // Generate fun company ID (e.g., ocean_elephant_614)
    console.log('[Auth] Generating fun company ID for beta user...');
    const funCompanyId = await generateUniqueFunCompanyId(db);
    console.log('[Auth] Generated fun company ID:', funCompanyId);
